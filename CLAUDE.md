...
| DST gate silently skips all workflows when ET hour is 08 or 09 | `$(( $H * 60 ))` treated `08`/`09` as invalid octal → `value too great for base` → comparison failed → `skip=true`. All 8 workflow files had the same bug. Fix (2026-06-25): `10#$H` prefix forces base-10 parsing. Affected any workflow arriving 8:00–9:59 AM ET (including morning session on Day 1 of Session 3). |
