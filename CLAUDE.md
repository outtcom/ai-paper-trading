...
| Fund Manager decision tree had no exception for a risk-team majority-reject — LMT opened 2026-07-30 despite `risk_assessment: "rejected"` (2 of 3 perspectives reduce/reject, overbought RSI 72.3, non-focus sector). STEP 2's mandatory-BUY rule (market_posture NEUTRAL + conviction ≥ medium → BUY, added 2026-05-19 to fix Session 1 cash drag) overrode even the model's own `final_reasoning`, which agreed with the reject and concluded "HOLD preserves dry powder." Contributed to Session 3's -13.86% return / 14.44% max drawdown by day 28. | Fix (2026-08-03): `agents/fund_manager.py` — added `risk_assessment == "rejected"` as STEP 1 condition 5 (absolute veto, independent of deployment posture). Also added a deterministic code-level backstop in `run()` (same pattern as the existing ETF allocation cap) that force-overrides `action` to `hold`/`qty=0` whenever the risk team rejected but the LLM still returned `"buy"` — so the veto holds even if the model doesn't follow the prompt. `"reduced"` was previously untouched by the code and still executed at full fund-manager size — `risk_adjusted_decision.final_position_size` was only read for Telegram display (`tools/telegram_bot.py`), never wired into actual sizing. Fixed (2026-08-03): `morning_session.py::_size_position` now caps `pos_frac` at `risk_final_position_size × 1.25`, applied after the deployment-urgency floor so urgency can't silently cancel out a risk-team reduction. |
