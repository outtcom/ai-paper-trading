...
def main():
    print(f"\n[preclose] ========== Pre-Close Alert {datetime.today().strftime('%Y-%m-%d')} ==========")
    ...
    for ticker, pos in positions.items():
        try:
            ...
            lines.append(
                ...
                f"  TP: ${tp:.2f}  (+{pct_to_tp:.1f}% away)\n"
                f"  SL: ${sl:.2f}  (-{pct_to_sl:.1f}% away)"
            )

            # Flag if within 0.5% of stop loss
            if pct_to_sl <= 0.5:
                danger.append(f"🚨 {ticker} is {pct_to_sl:.2f}% from stop loss!")
